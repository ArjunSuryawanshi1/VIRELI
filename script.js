...
const MOOD_OPTIONS = [
  { id: "good", label: "Good", text: "You have some energy today. Let’s use it well." },
  { id: "ok", label: "Okay", text: "Let’s build a realistic plan for today." },
  { id: "bad", label: "Bad", text: "Let’s make today more manageable." },
];
...
function getBlankSetupRoutineDraft(baseRoutine = EMPTY_ROUTINE_DRAFT) {
  return {
    ...baseRoutine,
    preferredDailyWorkloadMinutes: baseRoutine.preferredDailyWorkloadMinutes || "",
    preferredWorkIntervalMinutes: "",
    preferredWorkloadLabel: "",
...
const WORKLOAD_OPTIONS = [
  { id: "one-hour", label: "1 hour", minutes: 60 },
  { id: "two-hours", label: "2 hours", minutes: 120 },
  { id: "three-hours", label: "3 hours", minutes: 180 },
  { id: "four-hours", label: "4 hours", minutes: 240 },
  { id: "custom", label: "Custom", minutes: "" },
];
const WORK_INTERVAL_OPTIONS = [
...
const DURATION_PICKER_OPTIONS = Array.from({ length: 16 }, (_, index) => {
  const minutes = 30 + index * 15;
  return { value: String(minutes), label: formatDurationFromMinutes(minutes) };
});
const INTERVAL_PICKER_OPTIONS = [15, 20, 25, 30, 35, 40, 45, 50, 60, 75, 90].map((minutes) => ({
...
function loadSidebarState() {
  const sidebarState = readPersistentObject(SIDEBAR_STORAGE_KEY, EMPTY_SIDEBAR_STATE);
  return {
    open: sidebarState.open !== false,
    collapsed: Boolean(sidebarState.collapsed),
  };
}
...
function migrateWorkloadMinutes(entry = {}) {
  const explicitMinutes = String(entry.preferredDailyWorkloadMinutes || "").trim();
  if (explicitMinutes) {
    return explicitMinutes;
  }

  const legacyLabel = String(entry.preferredWorkloadLabel || "").toLowerCase();
  if (legacyLabel === "light") {
    return "120";
  }
  if (legacyLabel === "balanced") {
    return "180";
  }
  if (legacyLabel === "focused") {
    return "240";
  }
  return "";
}

function getWorkloadLabelFromMinutes(minutesValue) {
  const match = WORKLOAD_OPTIONS.find((option) => option.minutes && String(option.minutes) === String(minutesValue));
  return match?.label || (minutesValue ? "Custom" : "");
}

function getWorkIntervalLabelFromMinutes(minutesValue) {
  const match = WORK_INTERVAL_OPTIONS.find((option) => option.minutes && String(option.minutes) === String(minutesValue));
  return match?.label || (minutesValue ? "Custom" : "");
}
...
function normalizeRoutineEntry(entry = {}) {
  ...
  const routineCollections = normalizeRoutineCollections(
    Array.isArray(entry.routineCollections) && entry.routineCollections.length
      ...
      : dailyActivities.length
        ? [{ id: "collection-existing", name: "Routine", activities: dailyActivities }]
        : [],
  );
  ...
  return {
    ...
    preferredDailyWorkloadMinutes,
    preferredWorkIntervalMinutes,
    preferredWorkloadLabel: getWorkloadLabelFromMinutes(preferredDailyWorkloadMinutes),
    preferredWorkIntervalLabel: getWorkIntervalLabelFromMinutes(preferredWorkIntervalMinutes),
    weekdayWorkloadMinutes: String(entry.weekdayWorkloadMinutes || preferredDailyWorkloadMinutes || ""),
    weekendWorkloadMinutes: String(entry.weekendWorkloadMinutes || preferredDailyWorkloadMinutes || ""),
    createdAt: entry.createdAt || "",
    updatedAt: entry.updatedAt || "",
  };
}

function normalizeRoutineCollections(collections = []) {
  return collections
    .map((collection, collectionIndex) => ({
      id: collection.id || makeId(`routine-collection-${collectionIndex}`),
      name: String(collection.name || collection.title || "").trim(),
      activities: (Array.isArray(collection.activities) ? collection.activities : [])
        .map((activity, activityIndex) => ({
          id: activity.id || makeId(`routine-item-${activityIndex}`),
          name: String(activity.name || activity.label || "").trim(),
          days: String(activity.days || "Weekdays").trim(),
          usualTime: String(activity.usualTime || activity.preferredTime || activity.time || "").trim(),
...
function saveRoutine(routineDraft) {
  ...
  const nextRoutine = normalizeRoutineEntry({
    ...
    preferredWorkIntervalMinutes: String(routineDraft.preferredWorkIntervalMinutes || "").trim(),
    preferredWorkIntervalLabel: String(routineDraft.preferredWorkIntervalLabel || "").trim(),
...
function normalizeCalendarTask(task = {}) {
  ...
  const normalizedType = itemType === "Event" ? "Event" : "Task";
  return {
    ...
    flexible: task.fixed ? false : task.flexible !== false,
...
function homeworkItemToTask(item = {}) {
  return normalizeCalendarTask({
    id: item.sourceTaskId || `homework-task-${item.id || makeId("homework")}`,
    sourceHomeworkId: item.id || "",
    title: item.title || item.topic || "Untitled task",
    subject: item.classLabel || item.subject || "",
    itemType: "Task",
    type: "Task",
    scheduledDate: item.scheduledDate || item.dueDate || "",
    scheduledTime: item.scheduledTime || "",
    dueDate: item.dueDate || "",
    durationMinutes: item.estimatedMinutes || DEFAULT_TASK_DURATION_MINUTES,
...
function loadUnifiedCalendarTasks() {
  const savedTasks = loadCalendarTasks().map((task) =>
    task.itemType === "Meeting" ? normalizeCalendarTask({ ...task, itemType: "Task", type: "Task", fixed: true }) : task,
  );
  const existingKeys = new Set(savedTasks.map((task) => task.sourceHomeworkId || `${task.title.toLowerCase()}|${task.scheduledDate || task.dueDate || ""}`));
  const migratedHomeworkTasks = loadSavedHomework()
    .map(homeworkItemToTask)
    .filter((task) => {
      const key = task.sourceHomeworkId || `${task.title.toLowerCase()}|${task.scheduledDate || task.dueDate || ""}`;
  ...
  return [...savedTasks, ...migratedHomeworkTasks].slice(0, 80);
}
...
function loadConsistencyEvents() {
  return readPersistentArray(CONSISTENCY_STORAGE_KEY)
    .filter((entry) => entry && entry.type && entry.itemId)
    .slice(0, CONSISTENCY_EVENT_LIMIT);
}

function saveConsistencyEvents(entries = []) {
  writePersistentArray(CONSISTENCY_STORAGE_KEY, entries.slice(0, CONSISTENCY_EVENT_LIMIT));
}
...
function getTodayScheduleBlocks({
  ...
  const collectionActivities = (routine.routineCollections || [])
    .flatMap((collection) =>
...
function getCalendarItemDuration(item) {
  ...
  return item.itemType === "Meeting" || item.itemType === "Event" ? 60 : 45;
}
...
function getScheduleSourceLabel(source, item = {}) {
  if (source === "free") {
    return "Free time";
  }
  if (source === "routine") {
  ...
  if (source === "task") {
    return item.itemType === "Meeting" ? "Meeting" : item.itemType === "Event" ? "Event" : "Task";
  }
  if (source === "calendar") {
    return item.itemType === "Meeting" ? "Meeting" : item.itemType === "Event" ? "Event" : "Task";
  }
...
function getAskPromptChips({ routine = EMPTY_ROUTINE_DRAFT, homeworkItems = [], calendarTasks = [], mood = "" }) {
  const recommendation = getNextAssignmentRecommendation({ routine, homeworkItems, calendarTasks, mood });
  const freeWindow = recommendation.freeWindows[0];
  const openAssignment = homeworkItems.find((item) => !item.completed);
  const upcomingEvent = calendarTasks
    .filter((item) => !item.completed && (item.scheduledDate || item.dueDate) >= getDateInputValue())
    .sort((a, b) => `${a.scheduledDate || a.dueDate || "9999-12-31"} ${a.scheduledTime || ""}`.localeCompare(`${b.scheduledDate || b.dueDate || "9999-12-31"} ${b.scheduledTime || ""}`))[0];
  const chips = ["What should I do next?"];

  if (openAssignment?.title) {
    chips.push(`Plan ${openAssignment.title}`);
  } else {
    chips.push("Add a task for tomorrow");
  }
  if (freeWindow) {
    chips.push(`What fits in ${freeWindow.durationLabel}?`);
  }
  if (upcomingEvent?.title) {
    chips.push(`Work around ${upcomingEvent.title}`);
  }
  if (routine.preferredWorkIntervalMinutes) {
    chips.push(`Use ${formatDurationFromMinutes(Number(routine.preferredWorkIntervalMinutes))} blocks`);
  }
...
function getMoodLabel(mood) {
  return MOOD_OPTIONS.find((option) => option.id === mood)?.label || "";
}

function getRecommendationFactors({ item, recommendation, routine = EMPTY_ROUTINE_DRAFT, mood = "" }) {
  if (!item) {
    return [];
  }
  ...
  const workloadMinutes = getPreferredWorkloadMinutes(routine);
  const intervalMinutes = Number(routine.preferredWorkIntervalMinutes);
  const fixedCount = (routine.dailyActivities || []).filter((activity) => activity.name && activity.fixed !== false).length;

  if (item.dueDate) {
    factors.push(`Deadline: due ${formatShortDate(item.dueDate)}.`);
  }
  if (item.priority && item.priority !== "Normal") {
    factors.push(`Priority: marked ${item.priority}.`);
  }
  if (nextWindow) {
    factors.push(`Free time: ${nextWindow.label} is open.`);
  }
  if (mood) {
    factors.push(`Mood: ${getMoodLabel(mood)} check-in keeps the plan realistic.`);
  }
  if (workloadMinutes) {
    factors.push(`Workload hours: ${formatDurationFromMinutes(workloadMinutes)} preferred today.`);
  }
  if (Number.isFinite(intervalMinutes) && intervalMinutes > 0) {
    factors.push(`Preferred interval: ${formatDurationFromMinutes(intervalMinutes)} blocks.`);
  }
  if (fixedCount) {
    factors.push(`${fixedCount} fixed commitment${fixedCount === 1 ? "" : "s"} protected.`);
  }

  return factors.slice(0, 5);
}

function getConsistencyFactors(item, consistencyEvents = []) {
  if (!item) {
    return [];
  }

  const relatedEvents = consistencyEvents.filter((event) =>
    event.itemId === item.id ||
    (event.itemTitle && item.title && event.itemTitle.toLowerCase() === item.title.toLowerCase()),
  );
  const skippedCount = relatedEvents.filter((event) => /skipped|deleted/.test(event.type)).length;
  const rescheduledCount = relatedEvents.filter((event) => /rescheduled|moved/.test(event.type)).length;
  const completedCount = relatedEvents.filter((event) => /completed/.test(event.type)).length;
  const factors = [];

  if (completedCount) {
    factors.push(`Consistency: you have finished this kind of item before.`);
  }
  if (rescheduledCount) {
    factors.push(`Consistency: this has moved before, so VIRELI keeps the block realistic.`);
  }
  if (skippedCount) {
    factors.push(`Consistency: this has been skipped before, so one smaller start may work better.`);
  }
...
function getIntelligentStatus({ routine = EMPTY_ROUTINE_DRAFT, homeworkItems = [], calendarTasks = [], savedClasses = [] }) {
  ...
  if (freeWindows[0]) {
    return {
      label: `${freeWindows[0].durationLabel} free`,
...
function VireliLogoMark({ className = "", animated = false }) {
  return html`
    <div className=${cx("vireli-logo-mark", animated && "is-animated", className)} aria-label="VIRELI logo">
      <img src="./icons/vireli-icon.svg" alt="" aria-hidden="true" />
    </div>
  `;
}
  ...
  return html`
    ...
    >
      ...
      <div className="intro-title-shell opening-title-shell">
        <${motion.div}
          className="opening-brand-motion"
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-3xl flex-col items-center gap-5">
        ...
        >
          <div className="mood-heading mood-heading-centered">
            <p className="eyebrow">Setup 1 of 2 · Your Name</p>
          ...
          <div className="setup-form-stack">
            ...
            <div className="mood-control-row mood-control-row-centered">
              <button type="button" className="primary-button" onClick=${() => onContinue({ name: aboutName })} disabled=${!canContinue}>
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-4xl flex-col items-center gap-5">
        ...
        >
            ...
            ${PRIMARY_USE_OPTIONS.map(
              (option) => html`
                <button
                  key=${option}
                  type="button"
                  className=${cx("mood-choice setup-choice setup-mode-choice", selectedMode === option && "is-selected")}
          ...
          ${selectedMode
            ...
            : null}
          <div className="mood-control-row mood-control-row-centered">
            <button type="button" className="primary-button" onClick=${() => onContinue(profileDraft)} disabled=${!canContinue}>
              Continue
            </button>
          </div>
        </${motion.div}>
...
function getTimeValueFromPicker(parts) {
  let hour = Number(parts.hour || "7");
...
function TimeScrollPicker({ label, help = "", value, onChange }) {
  const parts = getTimePickerParts(value);
  const updatePart = (field, nextValue) => {
    onChange(getTimeValueFromPicker({ ...parts, [field]: nextValue }));
  };

  return html`
    <div className="scroll-picker-field">
      <span>${label}</span>
      ${help ? html`<small>${help}</small>` : null}
      <div className="scroll-picker-row" role="group" aria-label=${label}>
        <select
          className="scroll-picker-select"
          value=${parts.hour}
          onChange=${(event) => updatePart("hour", event.target.value)}
          aria-label=${`${label} hour`}
        >
          <option value="">Hour</option>
          ${TIME_PICKER_HOURS.map((hour) => html`<option key=${hour} value=${hour}>${hour}</option>`)}
        </select>
        <select
          className="scroll-picker-select"
          value=${parts.minute}
        ...
        >
          ...
          ${TIME_PICKER_MINUTES.map((minute) => html`<option key=${minute} value=${minute}>${minute}</option>`)}
        </select>
        <select
          className="scroll-picker-select is-period"
          value=${parts.period}
...
function DurationScrollPicker({ label, help = "", value, options, onChange }) {
  return html`
    <label className="scroll-picker-field">
      <span>${label}</span>
      ${help ? html`<small>${help}</small>` : null}
      <select
        className="scroll-picker-select scroll-picker-duration"
        value=${value}
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-5xl flex-col items-center gap-5">
        ...
        >
          <div className="mood-heading">
            <p className="eyebrow">Setup 2 of 2 · Schedule</p>
          ...
          <div className="routine-form">
            <div className="scroll-picker-grid">
              <div className="setup-picker-card">
                <${TimeScrollPicker}
                  label="Wake Time"
                  help="When do you normally wake up?"
                  value=${routineDraft.wakeTime}
                  onChange=${(value) => onRoutineChange("wakeTime", value)}
                />
              </div>
              <div className="setup-picker-card">
                <${TimeScrollPicker}
              ...
              <div className="setup-picker-card">
                <${DurationScrollPicker}
                  label="Available daily work time"
                  help="How much task time should VIRELI plan around? If you leave this blank, VIRELI uses 4 hours."
            ...
            ${canContinue
              ? html`
                  <${motion.div} className="mood-control-row" initial=${{ opacity: 0, y: 10 }} animate=${{ opacity: 1, y: 0 }} exit=${{ opacity: 0, y: 8 }}>
                    <button type="button" className="primary-button" onClick=${onContinue}>Finish Setup</button>
                  </${motion.div}>
                `
              : null}
          </${AnimatePresence}>
...
function LaunchCheckInScreen({ moodSelection, onMoodSelect }) {
  const selectedMood = moodSelection || "";
  ...
  return html`
    ...
    >
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-3xl flex-col items-center gap-5">
        <${motion.div}
          className="mood-panel mood-panel-centered launch-check-in-panel"
            ...
            ${MOOD_OPTIONS.map(
              (option) => html`
                <button
                  key=${option.id}
                  type="button"
                  className=${cx("mood-choice", selectedMood === option.id && "is-selected")}
                  aria-pressed=${selectedMood === option.id}
                  onClick=${() => onMoodSelect(option.id)}
                >
                  <span className="mood-choice-label">${option.label}</span>
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-5xl flex-col items-center gap-5">
        ...
        >
              ...
              ? dailyActivities.map(
                  (activity, index) => html`
                    <div key=${activity.id || `routine-activity-${index}`} className="routine-activity-row routine-builder-row">
                      <input className="planning-input" value=${activity.name} onInput=${(event) => onRoutineActivityChange(index, "name", event.target.value)} placeholder="Practice, workout, work hours..." />
                      ...
                      <button type="button" className="secondary-button" onClick=${() => onRoutineActivityRemove(index)}>Remove</button>
                    </div>
                  `,
                )
              : html`<div className="soft-note"><p>No routine items yet. Add one if there is something VIRELI should plan around.</p></div>`}
          </div>
          <div className="card-footer-row">
            ...
            <button type="button" className="primary-button" onClick=${onSaveRoutine}>Save routine</button>
          </div>
        </${motion.div}>
      </div>
    </${motion.section}>
...
function AccountScreen({
  ...
  const isSigningIn = accountStatus === "signing-in";
  const isCheckingUsername = accountStatus === "checking";
  const isBusy = isCreatingAccount || isSigningIn || isCheckingUsername;
  const canContinueUsername = Boolean(accountUsername.trim()) && !isBusy;
  ...
  return html`
    ...
    >
      <div className="relative z-10 mx-auto flex w-full max-w-lg flex-col items-center">
        ...
        >
          <div className="mood-heading auth-heading get-started-heading">
            <div className="get-started-logo-shell">
            ...
            : html`
                <form className="vireli-account-form" onSubmit=${onVireliAccountSubmit}>
                  ...
                  <label className="field-stack password-field-stack">
                    <span>${accountAuthMode === "signin" ? "Password" : "Create password"}</span>
                ...
                <button
                  ...
                  onClick=${onAccountPasswordVisibleToggle}
                >
                  ...
                  <div className="account-step-actions">
                    ...
                    >
                      ${isCreatingAccount ? "Creating account..." : isSigningIn ? "Signing in..." : accountAuthMode === "signin" ? "Sign in" : "Create account"}
                    </button>
                  </div>
                </form>
              `}
...
function HomeTab({
  ...
  const allTasks = [
    ...calendarTasks.filter((item) => !item.completed).map((item) => ({ ...item, sourceKind: "task" })),
    ...homeworkItems.filter((item) => !item.completed).map(homeworkItemToTask).map((item) => ({ ...item, sourceKind: "task" })),
  ].filter((task, index, tasks) =>
    tasks.findIndex((candidate) => candidate.id === task.id || (candidate.title === task.title && candidate.scheduledDate === task.scheduledDate)) === index,
  );
...
function ModeItemsTab({
  ...
  const visibleItems = calendarTasks
    .filter((item) => (item.itemType || item.type || "Task") === itemType)
  ...
  const pageLead = "Add what you need to accomplish. VIRELI will fit it around your schedule.";
            ...
            <div className="homework-form-grid">
              ...
              <label className="field-stack">
                ...
                <input
                  ...
                  onInput=${(event) => onCalendarTaskDraftChange("dueDate", event.target.value)}
                />
              </label>
            </div>
            <div className="card-footer-row">
    ...
    >
      ...
      <div className=${cx("daily-grid", isMeeting && "meeting-grid")}>
        <article className=${cx("feature-card feature-card-support plan-builder-card", isMeeting && "meeting-builder-card")}>
          <div className="card-topline card-topline-simple">
            <span className="mood-chip">${editingCalendarTaskId ? "Edit" : isMeeting ? "Meeting details" : "Task details"}</span>
          </div>
          ...
          <form className="calendar-add-form" onSubmit=${(event) => onModeItemSubmit(itemType, event)}>
            <div className="homework-form-grid">
              ...
              <label className="field-stack">
                <span>${isMeeting ? "Date" : "Date or deadline"}</span>
                <input
                  className="planning-input"
                  type="date"
                  value=${calendarTaskDraft.scheduledDate || selectedDate}
            ...
            ${isMeeting
              ? html`
                  <textarea
                    className="planning-input compact-textarea"
                    value=${calendarTaskDraft.notes}
                    onInput=${(event) => onCalendarTaskDraftChange("notes", event.target.value)}
                    rows="3"
                    placeholder="Location, call link, or notes."
                  ></textarea>
                `
              : null}
            <label className="settings-toggle-row calendar-toggle-row">
              <input
                type="checkbox"
                checked=${calendarTaskDraft.fixed}
                onChange=${(event) => {
                  onCalendarTaskDraftChange("fixed", event.target.checked);
                  onCalendarTaskDraftChange("flexible", !event.target.checked);
                }}
              />
            ...
            <div className="card-footer-row">
              ...
              <div className="calendar-form-actions">
                ${editingCalendarTaskId
                  ...
                  : null}
                <button type="submit" className="primary-button" disabled=${!calendarTaskDraft.title.trim()}>
                  ${editingCalendarTaskId ? "Save changes" : `Add ${itemType.toLowerCase()}`}
                </button>
              </div>
            </div>
          </form>
          ...
          ${visibleItems.length
            ? html`
                <div className="selected-day-list">
                  ${visibleItems.map(
                    (item) => html`
                      <div key=${item.id} className=${cx("selected-day-item", isMeeting ? "is-meeting" : "is-task", item.completed && "is-complete")}>
                        <div>
                          <strong>${item.title}</strong>
                          <span>${[
                            item.scheduledDate ? formatShortDate(item.scheduledDate) : "",
              ...
              `
            : html`<p>No ${pageTitle.toLowerCase()} yet.</p>`}
        </article>
      </div>
    </${motion.section}>
  `;
...
function CalendarTab({
  ...
  const calendarItems = [
    ...
    ...calendarTasks.map((task) => ({
      ...
      source: "task",
      sourceLabel: getScheduleSourceLabel("task", task),
    ...
    >
      <div className="tab-heading">
        <div>
          ...
          <h1 className="font-display">How can we improve VIRELI?</h1>
        </div>
      </div>

      <article className="feature-card improve-card">
        ${feedbackSubmitted
          ? html`<div className="status-banner status-banner-soft">Saved. Thank you for helping improve VIRELI.</div>`
          : null}
        <label className="field-stack">
          <span>Category</span>
          <select className="planning-input" value=${feedbackDraft.area} onChange=${(event) => onFeedbackAreaChange(event.target.value)}>
            <option value="">Choose a category</option>
            ${FEEDBACK_AREAS.map((area) => html`<option key=${area} value=${area}>${area}</option>`)}
          </select>
        </label>
        <label className="field-stack">
          <span>What should VIRELI improve?</span>
          <textarea
...
function SettingsTab({
  ...
  const settingsSections = [
    { id: "account", title: "Account", summary: accountSummary },
    { id: "profile", title: "Profile", summary: profile.name || "Name" },
    { id: "schedule", title: "Schedule", summary: "Wake, sleep, and task time" },
  ...
  const renderSettingsDetail = () => {
    ...
    if (activeSettingsSection === "profile") {
      return html`
        <div className="settings-detail-body">
          <p>Change the display name VIRELI uses inside the planner.</p>
          <div className="profile-form">
            <label className="field-stack">
              <span>Display name</span>
              <input
          ...
          <button type="button" className="primary-button" onClick=${onSaveProfileSettings} disabled=${!profileDraft.name.trim()}>
            Save profile
          </button>
        </div>
      `;
    ...
    if (activeSettingsSection === "schedule") {
      return html`
        <div className="settings-detail-body">
          <p>These values help VIRELI place tasks without filling every open minute.</p>
          <label className="field-stack">
    ...
    if (activeSettingsSection === "security") {
      return html`
        <div className="settings-detail-body">
          <p>${profile.authMode === "vireli-account" ? "Change your VIRELI account password." : "Create or sign into a VIRELI account to use password security."}</p>
          <form className="settings-password-form" onSubmit=${onPasswordSubmit}>
            <label className="field-stack">
              <span>Current password</span>
              <input className="planning-input" type="password" value=${passwordDraft.currentPassword} onInput=${(event) => onPasswordDraftChange("currentPassword", event.target.value)} placeholder="Current password" autoComplete="current-password" disabled=${profile.authMode !== "vireli-account" || passwordDraft.status === "saving"} />
  ...
  if (activeTab === "home") {
    ...
    activeView = html`
      <${SettingsTab}
        ...
        onSchedulingPreferenceChange=${onSchedulingPreferenceChange}
        onTabChange=${onTabChange}
      />
    `;
  }
...
function App() {
  ...
  const [launchSetupStep, setLaunchSetupStep] = useState(1);
  const [launchSetupComplete, setLaunchSetupComplete] = useState(false);
  ...
  const [calendarTasks, setCalendarTasks] = useState(loadUnifiedCalendarTasks);
  ...
  const [consistencyEvents, setConsistencyEvents] = useState(loadConsistencyEvents);
  ...
  const currentSetupStep = Math.max(1, Math.min(2, Number(launchSetupStep || 1)));
  ...
  function recordConsistencyEvent(type, item, extra = {}) {
    if (!item?.id) {
      return;
    }

    setConsistencyEvents((currentEvents) => [
      {
        id: makeId("consistency"),
        type,
        itemId: item.id,
        itemTitle: item.title || item.name || "",
        itemKind: item.itemType || item.type || "Item",
        scheduledDate: item.scheduledDate || item.dueDate || "",
        scheduledTime: item.scheduledTime || "",
        durationMinutes: item.estimatedMinutes || item.durationMinutes || "",
        createdAt: new Date().toISOString(),
        ...extra,
      },
      ...currentEvents,
  ...
  useEffect(() => {
    saveConsistencyEvents(consistencyEvents);
  }, [consistencyEvents]);
  ...
  useEffect(() => {
    ...
    fetch("/api/auth/me", { credentials: "same-origin" })
      ...
      .then((session) => {
        ...
        setProfileDraft({ ...sessionProfile, name: sessionProfile.name || "" });
      })
      .catch(() => {});
  }, []);
  ...
  function advanceSetupStep(nextStep, changes = {}) {
    const now = new Date().toISOString();
    const normalizedStep = Math.max(1, Math.min(2, Number(nextStep || 1)));
  ...
  function handleSetupModeContinue(modeDraft = profileDraft) {
    ...
    advanceSetupStep(2, updates);
  }

  function handleScheduleBasicsContinue() {
    const nextRoutine = saveRoutine({
      ...routineDraft,
      wakeTime: routineDraft.wakeTime || "07:00",
      bedTime: routineDraft.bedTime || "22:00",
  ...
  async function handleAccountUsernameContinue(event) {
    ...
    if (accountAuthMode === "create" && /^https?:$/.test(window.location.protocol)) {
      ...
      try {
        const response = await fetch("/api/auth/vireli/check-username", {
          method: "POST",
        ...
        if (!response.ok || !payload.available) {
          throw new Error(payload.error || "That username is already taken.");
        }
      } catch (error) {
        setAccountError(error.message || "That username is not available.");
  ...
  async function handleVireliAccountSubmit(event) {
    ...
    if (accountAuthMode === "create" && accountPassword !== accountPasswordConfirm) {
    ...
    setAccountStatus(accountAuthMode === "signin" ? "signing-in" : "creating");
    setAccountError("");

    try {
      const response = await fetch(accountAuthMode === "signin" ? "/api/auth/vireli/login" : "/api/auth/vireli/register", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "same-origin",
        body: JSON.stringify({
      ...
      if (!response.ok || !payload.authenticated || !payload.user) {
        throw new Error(payload.error || (accountAuthMode === "signin" ? "Could not sign in." : "Account could not be created."));
      }
      ...
      setAccountError(error.message || (accountAuthMode === "signin" ? "Could not sign in." : "Account could not be created."));
    } finally {
      setAccountStatus("idle");
    }
  }
  ...
  function handleSaveProfileSettings() {
    ...
    setProfile((currentProfile) => {
      const nextProfile = {
        ...currentProfile,
        connected: currentProfile.connected || !currentProfile.guest,
        name: profileDraft.name.trim() || currentProfile.name || "VIRELI user",
        primaryUse: profileDraft.primaryUse || currentProfile.primaryUse || "School",
        updatedAt: now,
      };
      setProfileDraft(nextProfile);
      return nextProfile;
  ...
  function handleRoutineIntervalSelect(option) {
    setRoutineDraft((currentDraft) => ({
      ...currentDraft,
      preferredWorkIntervalLabel: option.label,
      preferredWorkIntervalMinutes: option.id === "custom" ? "" : String(option.minutes),
    }));
  }
  ...
  function handleRoutineActivityAdd() {
    setRoutineDraft((currentDraft) => ({
      ...currentDraft,
      dailyActivities: [
        ...((currentDraft.dailyActivities || []).length ? currentDraft.dailyActivities : []),
        {
  ...
  function handleHomeworkCompleteToggle(homeworkId) {
    ...
    if (previousItem) {
      recordConsistencyEvent(previousItem.completed ? "assignment-reopened" : "assignment-completed", previousItem);
      showUndoToast(
  ...
  function handleHomeworkReschedule(homeworkId, scheduledDate, scheduledTime) {
    ...
    if (previousItem) {
      recordConsistencyEvent("assignment-rescheduled", previousItem, { nextDate: scheduledDate, nextTime: scheduledTime });
  ...
  function handleHomeworkDelete(homeworkId) {
    ...
    if (previousItem) {
      recordConsistencyEvent("assignment-skipped", previousItem);
  ...
  function handleModeItemSubmit(itemType, event) {
    ...
    setCalendarTasks((currentTasks) => {
      if (editingCalendarTaskId) {
        return currentTasks.map((task) => (task.id === editingCalendarTaskId ? nextTask : task));
      }

      return [nextTask, ...currentTasks].slice(0, 60);
    });
    setEditingCalendarTaskId("");
    if (nextTask.scheduledDate) {
      setCalendarMonth(new Date(`${nextTask.scheduledDate}T12:00:00`));
    ...
    setCalendarTaskDraft({
      ...
      itemType,
      type: itemType,
      fixed: itemType === "Meeting",
      flexible: itemType !== "Meeting",
    });
  }
  ...
  function handleCalendarTaskToggle(taskId) {
    const previousTask = calendarTasks.find((item) => item.id === taskId);
    setCalendarTasks((currentTasks) =>
    ...
    if (previousTask) {
      recordConsistencyEvent(previousTask.completed ? "item-reopened" : "item-completed", previousTask);
    }
  }
  ...
  function handleCalendarTaskDelete(taskId) {
    ...
    if (previousTask) {
      recordConsistencyEvent("item-skipped", previousTask);
    }
  }
...
function classifyAskPlannerType(prompt) {
    return "Task";
  }

  function detectAskSubject(prompt) {
    const normalizedPrompt = prompt.toLowerCase();
    const savedMatch = savedClasses.find((schoolClass) =>
      normalizedPrompt.includes(schoolClass.label.toLowerCase()),
    );

    if (savedMatch) {
      return { classId: savedMatch.id, classLabel: savedMatch.label };
    }
    ...
    const subject = fallbackSubjects.find((label) => normalizedPrompt.includes(label.toLowerCase()));
    return { classId: "", classLabel: subject || "" };
  }
  ...
  function buildAskCalendarAction(prompt) {
    ...
    const plannerType = classifyAskPlannerType(prompt);

    if (plannerType === "Assignment") {
      const subject = detectAskSubject(prompt);
      const estimatedMinutes = estimateAssignmentMinutes(title, prompt);
      const duplicateAssignment = homeworkItems.find(
        (item) =>
          !item.completed &&
          item.title.toLowerCase() === title.toLowerCase() &&
          (item.dueDate || item.scheduledDate || "") === scheduledDate,
      );

      if (duplicateAssignment) {
        return {
          needsDetail: true,
          reply: `${duplicateAssignment.title} is already saved as an assignment for ${formatShortDate(scheduledDate)}.`,
        };
      }
      ...
      return {
        ...
        assignment: {
          ...
          type: "Assignment",
          frequency,
          steps: createHomeworkSteps(title, prompt),
          guidance: buildHomeworkGuidance({ classLabel: subject.classLabel, topic: title, title, details: "" }),
          completed: false,
          completedAt: "",
          createdAt: new Date().toISOString(),
          updatedAt: new Date().toISOString(),
        },
        reply: `I found this: Add "${title}" as an assignment ${dateCopy}at ${formatTimeLabel(suggestedTime)}. Should I add it?`,
      };
    }
    ...
    return {
      ...
      task: {
        ...
        scheduledTime: suggestedTime,
        frequency,
        itemType: plannerType,
        type: plannerType,
        fixed: plannerType === "Meeting",
        flexible: plannerType !== "Meeting",
        notes: "Added from Ask VIRELI.",
        completed: false,
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      },
      reply: `I found this: Add "${title}" as a ${plannerType.toLowerCase()} ${frequencyCopy}${dateCopy}at ${formatTimeLabel(suggestedTime)}. Should I add it to your calendar?`,
    };
  }
  ...
  function handlePendingScheduleAnswer(prompt) {
    ...
    if (/^(yes|yeah|yep|sure|ok|okay|add it|please do)\b/.test(normalizedPrompt)) {
      ...
      if (pendingScheduleSuggestion.action === "remove") {
        const removedTask = calendarTasks.find((task) => task.id === pendingScheduleSuggestion.targetTaskId);
        setCalendarTasks((currentTasks) =>
          currentTasks.filter((task) => task.id !== pendingScheduleSuggestion.targetTaskId),
        );
        if (removedTask) {
          recordConsistencyEvent("ask-removed-item", removedTask);
        }
        reply = `Removed ${pendingScheduleSuggestion.targetTitle} from your calendar.`;
      } else if (pendingScheduleSuggestion.action === "move") {
        const movedTask = calendarTasks.find((task) => task.id === pendingScheduleSuggestion.targetTaskId);
        setCalendarTasks((currentTasks) =>
          currentTasks.map((task) =>
        ...
        if (movedTask) {
          recordConsistencyEvent("ask-moved-item", movedTask, {
            nextDate: pendingScheduleSuggestion.scheduledDate,
            nextTime: pendingScheduleSuggestion.scheduledTime,
          });
        }
      ...
      } else if (pendingScheduleSuggestion.action === "add-assignment") {
        const nextAssignment = {
          ...pendingScheduleSuggestion.assignment,
          updatedAt: now,
        };
        setHomeworkItems((currentItems) => [nextAssignment, ...currentItems].slice(0, 80));
        ...
        recordConsistencyEvent("ask-added-item", nextTask);
  ...
  function handleFeedbackSubmit() {
    ...
    writePersistentArray(FEEDBACK_STORAGE_KEY, [
      {
        id: makeId("feedback"),
        area: feedbackDraft.area || "General",
        text,
        page: activeTab,
  ...
  return html`
    ...
    >
      ...
      <${AnimatePresence} mode="wait">
        ${!introComplete
          ? html`<${OpeningScreen} key="opening" />`
          : !profileStepComplete
          ? html`
                <${AccountScreen}
                  key="account"
                  ...
                  onContinueToVireli=${handleContinueToVireli}
                  onAccountAuthModeChange=${handleAccountAuthModeChange}
                  ...
                  onAccountPasswordConfirmChange=${(value) => {
                    setAccountError("");
                    setAccountPasswordConfirm(value);
                  }}
                  onAccountPasswordVisibleToggle=${() => setAccountPasswordVisible((currentValue) => !currentValue)}
                  onVireliAccountSubmit=${handleVireliAccountSubmit}
          ...
          : !moodCheckInComplete
            ? html`
                ...
                />
              `
          : !launchSetupComplete && currentSetupStep === 1
            ? html`
                <${SetupAboutScreen}
                  key="setup-about"
                  profileDraft=${profileDraft}
          ...
          : html`
                <${DashboardShell}
                  ...
                  onRoutineIntervalSelect=${handleRoutineIntervalSelect}
                  ...
                  onSchedulingPreferenceChange=${handleSchedulingPreferenceChange}
                />
              `}
      </${AnimatePresence}>
